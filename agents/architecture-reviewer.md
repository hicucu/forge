...
변경된 diff 또는 파일 목록을 받아 아키텍처 관점에서 리뷰한다. 단독 실행 시 git diff를 입력으로 받고, feature-pipeline 내 Phase 4에서 호출될 때는 Phase 2~3 변경 파일 목록을 입력으로 받는다.
...
2. **스택 추론 우선**: 입력의 처음 단계에서 언어·아키텍처 패턴·프레임워크 파악 후 적용
