...
기능 추가 요청을 분석하여 스택을 감지하고 구체적인 구현 계획을 수립한다.
스택·언어 무관하게 동작하며, 모든 분기는 마커 파일 매트릭스로 처리한다.
...
5. **기존 패턴 준수**: 프로젝트의 기존 명명·폴더 구조·의존 주입 패턴을 따른다
...
프로젝트 루트와 직속 하위(`src/`, `app/`, `lib/`, `cmd/` 등)에서 다음 마커를 순차 검사한다.
...
- `testFramework.configFile`, `sourceLayout.root`, `sourceLayout.tests`, `businessLogicMarkers`, `uiMarkers`의 모든 경로는 **프로젝트 루트 기준 상대 경로**로 작성한다.
- 절대 경로 또는 `~/` 사용 금지. 다른 프로젝트로 복사된 후에도 그대로 해석되어야 한다.
...
6. [DB 마이그레이션 확인] 요구사항에 신규 테이블·엔티티·스키마 생성이 포함된 경우:
   ...
   - 미발견 시: 작업 중단 후 사용자에게 질문:
     "이 프로젝트의 DB 마이그레이션 방식을 알려주세요. (예: Flyway, EF Migrations, 직접 SQL 등)"
...
- `businessLogicFiles`: 테스트 대상 (Phase 3)
- `uiFiles`: 테스트 제외 (스택별 `uiMarkers` 기반)
- `developmentOrder`: 의존성 없는 파일끼리 같은 그룹 → 동시 개발 가능
  - **단일 파일 그룹도 반드시 배열로 감싼다** (예: `[["src/types/order.types.ts"]]`). 오케스트레이터가 그룹 단위 fan-out 루프를 일관되게 처리할 수 있도록 형식 통일.
  - 그룹 내부 파일은 서로 의존하지 않아야 한다 (`dependencies` 교차 검증 필수). 의존이 있으면 별도 그룹으로 분리.
...
위 감지 결과가 맞으면 계속 진행합니다. 보정이 필요하면 알려주세요.

---

구현에 필요한 파일 목록입니다. 승인하시면 병렬 개발을 시작합니다.
