...
graphifyy(pip) 또는 codegraph(npm) 중 하나를 선택하여 전 과정을 자동 수행한다.
...
실패(exit code ≠ 0)이면 아래를 출력하고 종료한다.
...
이후 단계에서 `pip` 표기는 실제 탐지된 명령(`pip` 또는 `pip3`)으로 대체한다.

### 0-3. 도구 선택

인수에 `graphify` 또는 `codegraph`가 명시된 경우 해당 도구를 사용한다.

인수가 없는 경우 아래 순서로 자동 감지한다.
...
이후 모든 Phase는 선택된 도구(`TOOL = graphify | codegraph`)에 따라 분기하여 실행한다.

### 0-4. 현재 설정 상태 파악

선택된 `TOOL`에 따라 아래 상태 변수를 기록한다.
...
`claude_integrated` 판정 시 전역(`~/.claude/settings.json`) → 프로젝트(`.claude/settings.json`) 순으로 확인하며 어느 쪽이든 등록되어 있으면 `true`로 처리한다.

`check` 인수로 실행한 경우: 선택된 도구 형식으로 상태를 출력하고 종료한다.
...
`check` 인수 없이 `reset` 인수만 있는 경우: 모든 상태 변수를 `false`로 재설정하여 전체를 재실행한다.

---

## Phase 1: 패키지 설치

`installed = false`인 경우에만 실행한다.

**graphifyy:**

pip 또는 pip3가 없으면 아래를 출력하고 종료한다.
...
`graph_exists = false` 또는 `reset` 모드인 경우 실행한다.
...
완료 후 `graphify-out/GRAPH_REPORT.md` 존재로 성공 확인한다.
...
완료 후 `.codegraph/codegraph.db` 존재로 성공 확인한다.
...
실패 시 에러를 출력하되 이후 Phase는 계속 진행한다.
...
`claude_integrated = false`인 경우에만 실행한다.
...
먼저 전역 설정에 codegraph MCP가 이미 등록되어 있는지 확인한다.
...
**전역에 없는 경우** — 프로젝트 `.claude/settings.json`에 추가한다.

1. `.claude/settings.json`이 없으면 `{}` 내용으로 새로 생성한다.
2. 기존 `mcpServers` 섹션이 없으면 추가한다.
3. `codegraph` 키가 이미 `mcpServers`에 있으면 건너뜀.
4. 아래 항목을 `mcpServers`에 추가한다:
...
실패 시 에러를 출력하고 이후 Phase는 계속 진행한다.
...
`hook_installed = false`인 경우에만 실행한다.
...
`.git/hooks/post-commit` 파일이 없으면 shebang 포함 새 파일로 생성, 있으면 기존 내용을 유지하면서 아래 블록을 파일 끝에 추가한다.
...
`gitignore_set = false`인 경우에만 실행한다.
...
`graphify-out/*` 또는 `graphify-out/` 항목이 이미 있으면 추가하지 않는다.
...
모든 Phase 실행 후 선택된 도구에 따라 아래 형식으로 요약을 출력한다.
