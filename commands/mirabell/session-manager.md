...
`~/.claude/projects/` 하위의 모든 JSONL 세션을 최대 10개 서브에이전트로 병렬 분석한다.
...
**원칙**: 파일 삭제·내용 변경 등 되돌리기 어려운 작업이 아닌 모든 작업은 사용자 확인 없이 자동 허가한다.
...
- 확인(`y`/`yes`) 없이는 어떤 경우에도 파일을 삭제하지 않는다

---

## Phase 0: 입력 파싱

커맨드 호출 시 전달된 인수를 파싱한다.
...
인수 없이 호출(`/session-manager`)하면 목록을 먼저 표시하고 사용자 추가 입력을 기다린다.
인수와 함께 호출하면 Phase 1 분석 후 해당 작업을 바로 실행한다.

---

## Phase 1: 세션 파일 수집

아래 명령으로 모든 JSONL 파일 경로와 파일 통계를 수집한다.
...
- 프로젝트명 디코드 (표시용): `D--deul-agent-by-deul` → `deul-agent-by-deul` (드라이브 접두사 `X--` 제거)
...
전체 N개 파일을 **최대 10개 배치**로 균등 분할한다 (배치당 ceil(N/10)개).

**필수: 아래 서브에이전트 호출을 단일 응답에 동시에 포함한다. 각 Agent 호출 시 `model: "haiku"` 를 지정한다.**
...
[
  {
    "sessionId": "4fe69104-0649-47f4-8334-2905a5de1261",
    "project": "D--deul-agent-by-deul",
...
배치가 1개뿐이면 서브에이전트 없이 직접 처리한다.

---

## Phase 3: 결과 병합 및 목록 표시

서브에이전트 결과를 수집하여 **최신 lastActiveAt 내림차순**으로 정렬한다.
커스텀 이름(`~/.claude/.session-names.json`)이 있는 세션은 헤더 줄 끝에 `"이름"` 형태로 표시한다.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 #   생성일       마지막 작업    교환   프로젝트
─────────────────────────────────────────────────────────────
  1  2026-05-14  2026-05-14      23   agent_by_deul
...
`delete <자연어 조건>` — 번호가 아닌 텍스트가 전달된 경우 조건으로 해석한다.
...
조건에 해당하는 세션 목록을 먼저 표시하고 확인을 구한다.
...
삭제 대상 세션 (N개):
  ...
  #1  2026-05-08  2026-05-08   15교환  agent_by_deul
...
3. `N개 세션이 삭제되었습니다.` 출력
4. 아래 질문을 출력하고 사용자 입력을 기다린다:

```
삭제된 세션을 제외한 목록을 다시 표시할까요? [y/N]
```

- `y` 또는 `yes` 입력 시: 이미 수집된 세션 데이터에서 삭제된 항목을 제거한 후 Phase 3 형식으로 목록을 재출력한다. JSONL 파일 재스캔 없이 메모리 내 데이터를 재사용한다.
- 그 외 입력: 종료

그 외 입력(`n`, `N`, `no`, Enter 등)은 `삭제가 취소되었습니다.` 출력 후 종료.

**절대 규칙: 사용자 명시적 확인(`y`/`yes`) 없이 파일을 삭제하지 않는다. 조건이 자명해 보여도 예외 없음.**
