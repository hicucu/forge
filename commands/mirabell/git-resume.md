...
diff가 500줄을 초과하면 파일별로 분할하여 읽는다:
...
20개를 초과하면 최근 20개만 분석하되, 나머지 커밋의 메시지 목록만 별도로 보여준다.

### 바이너리 파일 변경

이미지·DB 파일 등 바이너리 변경은 파일명과 크기 변화만 표시하고 diff는 생략한다.

### merge 커밋

`--no-merges` 플래그로 기본적으로 제외한다. 명시적으로 merge commit을 포함하려면 `--merges` 인수를 추가한다.

### 다른 작성자의 커밋 포함

기본은 `git config user.name` 기준이지만, `--all-authors` 인수를 주면 모든 작성자의 커밋을 포함한다.
