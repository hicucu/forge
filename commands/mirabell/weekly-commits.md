...
> **실행 원칙**: 모든 단계를 사용자에게 표시하지 않고 조용히 수행한다. 최종 마크다운 결과만 출력한다.

---

## Step 1: AUTHOR 결정

아래 우선순위로 `AUTHOR`를 결정한다.
...
실패 시 "현재 디렉토리가 git 저장소가 아닙니다." 출력 후 종료.
...
`REPO_ROOT`에서 아래를 순서대로 확인한다. 하나라도 해당하면 `IS_MONOREPO=true`.
...
모노레포인 경우 `PROJECT_ROOTS`를 수집한다 — 존재하는 폴더(`packages`, `apps`, `libs`) 각각의 직계 하위 디렉토리 이름 목록.
...
`--name-only`로 각 커밋이 수정한 파일 목록을 함께 가져온다 (모노레포 프로젝트 판별용).
...
Step 6 출력 직후, 수집된 전체 커밋 메시지를 Claude가 직접 분석하여 정리 요약을 출력한다.

### 분석 규칙

- **의미 중복 제거**: 동일하거나 실질적으로 같은 작업을 가리키는 메시지는 하나로 합친다
  - 예: "fix: 로그인 버그 수정", "fix: 로그인 오류 재수정" → "로그인 버그 수정"
- **관련 작업 묶기**: 같은 기능/컴포넌트에 대한 여러 커밋은 하나의 항목으로 통합한다
  - 예: "feat: 결제 UI 추가", "style: 결제 UI 스타일 조정", "fix: 결제 UI 버그" → "결제 UI 구현"
- **Conventional Commits 접두어 제거**: `feat:`, `fix:`, `chore:`, `refactor:` 등 타입 접두어를 제거하고 핵심 내용만 남긴다
- **날짜·SHA 미포함**: 정리 요약에는 날짜와 SHA를 표시하지 않는다
