...
변경 브랜치를 4개 전문 에이전트가 병렬로 검토하고 aggregator가 통합 리포트를 작성한다.
feature-pipeline의 Phase 4와 동일한 자산을 사용하여 단독 실행한다.
...
소스 코드 변경이 없으면 "리뷰할 코드 변경사항이 없습니다" 출력 후 종료.
...
diff에 신규 테이블·엔티티·스키마 생성이 포함된 경우, security-reviewer에게 마이그레이션 패턴 확인을 명시적으로 지시한다.
...
- 전문 에이전트 정의: `feature-pipeline/.claude/agents/{architecture,security,performance,style}-reviewer.agent.md`
- 통합 로직: `feature-pipeline/.claude/agents/review-aggregator.agent.md`
- 파이프라인 흐름: `feature-pipeline/.claude/skills/feature-pipeline/SKILL.md` Phase 4
