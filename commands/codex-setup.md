...
이 파일(`commands/codex-setup.md`)이 위치한 디렉토리의 부모 디렉토리가 forge 루트입니다.
플러그인 캐시 경로를 확인합니다:

Windows (PowerShell):

```powershell
$forgeRoot = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
# 또는 직접 확인:
ls "$HOME\.claude\plugins\cache\hicucu\forge\"
```
...
설치된 버전 디렉토리(예: `1.2.0/`)를 확인하고 `.codex/agents/` 경로를 파악합니다.
...
```powershell
$forgeAgentsDir = "<FORGE_ROOT>\.codex\agents"
$codexAgentsDir = "$HOME\.codex\agents"
New-Item -ItemType Directory -Force -Path $codexAgentsDir | Out-Null
Get-ChildItem "$forgeAgentsDir\*.toml" | ForEach-Object {
    Copy-Item $_.FullName "$codexAgentsDir\forge-$($_.Name)"
}
$count = (Get-ChildItem "$codexAgentsDir\forge-*.toml").Count
Write-Host "forge 에이전트 $count개 등록 완료"
```

실제 복사 (Linux/macOS Bash):

```bash
forge_agents="<FORGE_ROOT>/.codex/agents"
codex_agents="$HOME/.codex/agents"
mkdir -p "$codex_agents"
count=0
for f in "$forge_agents"/*.toml; do
    cp "$f" "$codex_agents/forge-$(basename "$f")"
    count=$((count + 1))
done
echo "forge 에이전트 ${count}개 등록 완료"
```
...
```powershell
$agentCount = (Get-ChildItem "$HOME\.codex\agents\forge-*.toml" -ErrorAction SilentlyContinue).Count
Write-Host "~/.codex/agents/에 forge 에이전트 $agentCount개 확인"
```

Linux/macOS (Bash):

```bash
count=$(ls ~/.codex/agents/forge-*.toml 2>/dev/null | wc -l)
echo "~/.codex/agents/에 forge 에이전트 ${count}개 확인"
```
...
- ~/.codex/agents/: forge 에이전트 등록 완료 (4단계 출력 참조)
