...
지정 경로(기본: 현재 디렉토리)를 스캔하여 프로젝트에 최적화된 `.claudeignore` 파일을 생성한다.
기존 파일이 있으면 덮어쓰지 않고 누락 항목만 병합한다.
...
대상 경로에서 `.claudeignore` 존재 여부를 확인한다.
...
`reset` 시 `.claudeignore.bak`이 이미 존재하면 `.claudeignore.bak2`, `.claudeignore.bak3` 형식으로 넘버링한다.
...
대상 경로에서 아래 마커 파일 존재 여부를 확인하고 해당 패턴을 수집한다.
복수 마커가 발견되면 모두 수집한다.
...
마커 감지와 무관하게 대상 경로에서 실제 존재 여부를 직접 확인하여 추가 수집한다.
...
수집된 패턴을 아래 섹션 순서로 조립한다.

**조립 규칙:**

- 이미 기존 `.claudeignore`에 있는 패턴은 중복 추가하지 않는다.
- 각 섹션은 해당 패턴이 하나라도 있을 때만 출력한다.
- 스택별 패턴이 기본 섹션과 겹치면 기본 섹션에 합산하고 스택 섹션은 비워둔다.

**중복 판정 알고리즘**: 패턴을 정규화(trailing `/` 제거, `**` → `*` 단순화, 대소문자 무시)한 뒤 집합(set) 비교로 중복 여부 판정한다. 예: 기존 `node_modules/`와 신규 `node_modules/**`는 동일 패턴으로 간주해 신규 항목을 추가하지 않는다.
...
해당 패턴이 없는 섹션(스택별 추가, 실제 존재 확인)은 출력하지 않는다.
...
추가할 항목이 하나도 없으면 파일 수정 없이 Phase 4로 이동한다.
...
- Phase 2-1에서 조립된 전체 내용을 `<대상경로>/.claudeignore`에 작성한다.
...
- 기존 파일 끝에 빈 줄 1개를 삽입한 후 Phase 2-2 섹션을 추가한다.

**check 모드:**

- 파일 수정 없이 Phase 4로 이동한다.
...
| 마커 파일 전혀 없음                      | 스택별 섹션 생략, 공통 기본 항목만 생성 + "스택을 감지하지 못했습니다" 안내 |
