...
forge의 스킬·에이전트·파이프라인이 의도대로 동작하는지 검증하는 테스트 모음입니다.
...
- **결정론적 테스트**: Node.js (brainstorm-server), Bash
- **LLM 동작 테스트**: `claude` CLI가 PATH에 있어야 함 (`claude --version` 동작)
  - 대부분의 러너가 저장소 루트를 `--plugin-dir`로 자동 전달하므로 별도 플러그인 설치는 불필요
  - 러너는 `--dangerously-skip-permissions`로 실행되며 로그를 `/tmp/forge-tests/<timestamp>/` 에 남김
...
| 디렉토리                   | 분류        | 검증 대상                                      | 진입점                       |
| -------------------------- | ----------- | ---------------------------------------------- | ---------------------------- |
| `brainstorm-server/`       | 결정론적    | 브레인스토밍 WebSocket 서버 (프로토콜·수명주기) | `npm test`                   |
...
```bash
# 브레인스토밍 서버 단위 테스트 (Node)
cd tests/brainstorm-server
npm install
npm test                              # server.test.js 실행
node ws-protocol.test.js              # WebSocket 프로토콜 (개별)
bash windows-lifecycle.test.sh        # Windows 수명주기 (개별)
...
bash run-test.sh svelte-todo --plugin-dir /path/to/forge
```
...
# 압박 시나리오 (baseline은 위반해야 정상, forge 설치 후엔 준수해야 정상)
...
- 로그는 `/tmp/forge-tests/<timestamp>/` 에 쌓이므로 디스크 정리 시 참고하세요.
