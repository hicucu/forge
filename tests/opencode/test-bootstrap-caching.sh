...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$FORGE_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$FORGE_SKILLS_DIR/using-forge/SKILL.md" "$TEST_HOME/using-forge.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$FORGE_PLUGIN_FILE" missing
}
