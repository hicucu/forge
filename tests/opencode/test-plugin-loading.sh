...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/forge.js"
...
skill_count=$(find "$FORGE_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $FORGE_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-forge skill exists (critical for bootstrap)
echo "Test 3: Checking using-forge skill (required for bootstrap)..."
if [ -f "$FORGE_SKILLS_DIR/using-forge/SKILL.md" ]; then
...
if node --check "$FORGE_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/forge/' "$FORGE_PLUGIN_FILE"; then
