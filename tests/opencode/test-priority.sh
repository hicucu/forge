...
mkdir -p "$FORGE_SKILLS_DIR/priority-test"
cat > "$FORGE_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Forge version of priority test skill
---
# Priority Test Skill (Forge Version)

This is the FORGE version of the priority test skill.

PRIORITY_MARKER_FORGE_VERSION
EOF
...
if [ -f "$FORGE_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Forge version exists"
else
    echo "  [FAIL] Forge version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled forge priority
echo ""
echo "Test 2: Documenting personal vs forge priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled forge version instead of personal version"

# Test 3: Document project vs bundled forge priority
echo ""
echo "Test 3: Documenting project vs personal/forge priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled forge version instead of project version"

# Test 4: Test a non-colliding bundled forge skill is still available
echo ""
echo "Test 4: Testing non-colliding forge skill remains available..."

mkdir -p "$FORGE_SKILLS_DIR/forge-only-test"
cat > "$FORGE_SKILLS_DIR/forge-only-test/SKILL.md" <<'EOF'
---
name: forge-only-test
description: Forge-only priority test skill
---
# Forge Only Test Skill

PRIORITY_MARKER_FORGE_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"forge-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_FORGE_ONLY_VERSION" "Non-colliding forge skill is still registered"
