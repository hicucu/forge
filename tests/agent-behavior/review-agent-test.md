...
`review-agent`가 4개 전문 reviewer를 **병렬**로 실행하고, aggregator로 통합 보고서를 생성하며, Critical 이슈 발견 시 `issue-fixer-agent`를 올바르게 호출하는지 확인.
...
| Critical 1건+ | NEEDS_FIXES → issue-fixer-agent 호출됨 |

## issue-fixer 연동 검증 (Critical 있는 경우)

- [ ] issue-fixer-agent가 Critical 이슈 파일별로 호출됨
