...
digraph request_dispatch {
    ...
    "버그·에러·예상치 못한 동작?" -> "DEBUG" [label="yes"];
    "버그·에러·예상치 못한 동작?" -> "파일 1-2개·설계 불필요·10분 이내?" [label="no"];
    "파일 1-2개·설계 불필요·10분 이내?" -> "SIMPLE" [label="yes"];
    "파일 1-2개·설계 불필요·10분 이내?" -> "FULL" [label="no"];
}
```
...
digraph complex_pipeline {
    ...
    "요청 수신" -> "이어서/계속?";
    "이어서/계속?" -> "pipeline-state.md 확인" [label="yes"];
