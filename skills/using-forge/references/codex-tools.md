...
Codex 설정(`~/.codex/config.toml`)에 다음을 추가합니다:
...
이렇게 하면 `dispatching-parallel-agents`, `subagent-driven-development` 같은 스킬을 위한 `spawn_agent`, `wait_agent`, `close_agent`가 활성화됩니다.

레거시 참고: `rust-v0.115.0` 이전 Codex 빌드는 파견된 에이전트의 대기를
`wait`로 노출했습니다. 현재 Codex는 파견된 에이전트에 `wait_agent`를 사용합니다.
이제 `wait`라는 이름은 code-mode `exec/wait`에 속하며, 이는 yield된 exec
셀을 `cell_id`로 재개하는 도구입니다. 파견 에이전트의 결과 도구가 아닙니다.

## 환경 감지

워크트리를 생성하거나 브랜치를 마무리하는 스킬은, 진행 전에
읽기 전용 git 명령으로 환경을 감지해야 합니다:
