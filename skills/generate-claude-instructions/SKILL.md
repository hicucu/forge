...
description: 사용자의 입력(기존 지침 파일/프로젝트 폴더/없음)을 기반으로 CLAUDE.md와 참조 문서(DEVELOPMENT.md, LANGUAGE_GUIDELINES_{LANG}.md 언어별 분리, AI_BEHAVIOR.md, COMMIT_CONVENTION.md)를 현재 작업 디렉토리의 instruction/ 폴더에 생성하는 오케스트레이터. "CLAUDE.md 만들어줘", "지침서 생성", "instruction 문서 작성", "이 프로젝트 분석해서 지침 만들어줘", "기존 지침 참조해서 새로 써줘", "지침서 갱신", "AI 동작 지침 재생성", "개발원칙 문서 업데이트", "언어별 지침 다시 써줘", "전역 CLAUDE.md 만들어줘", "참조 문서 생성", "지침서 보완" 등 CLAUDE 지침 문서 생성·갱신 요청 시 반드시 사용. 다시 실행, 재생성, 업데이트, 보완, 부분 수정 요청에도 적용.
---

# CLAUDE Instruction Generator — 오케스트레이터

CLAUDE.md와 참조 문서를 사용자의 입력 컨텍스트(기존 지침 파일 또는 프로젝트 폴더)를 기반으로 생성한다. 모든 산출물은 현재 작업 디렉토리(CWD)의 `instruction/` 폴더에 저장된다.
...
언어/프레임워크 파일은 프로젝트 스택에서 **감지된 것만** 생성한다.
...
이 스킬과 동일한 `.claude/` 부모 디렉토리 내의 `.claude/agents/`에서 에이전트 정의 파일을 읽는다. 팀이 어디로 복사되든 상대 위치로 작동.
...
사용자 메시지에서 입력 참조를 추출하여 모드를 결정한다:
...
입력 참조가 식별되면 사용자에게 한 문장으로 알린다 (예: "프로젝트 폴더 `D:\my-project`를 분석하여 지침을 작성합니다").
...
에이전트 정의: `{팀_위치}/agents/dev-principles.agent.md`

프롬프트 골격:

```
{팀_위치}/agents/dev-principles.agent.md를 읽고 그 지침에 따라 작업한다.
...
에이전트 정의: `{팀_위치}/agents/language-guidelines.agent.md`

단일 파일이 아닌 **언어/프레임워크별 별도 파일**로 출력한다.

프롬프트 골격:

```
{팀_위치}/agents/language-guidelines.agent.md를 읽고 그 지침에 따라 작업한다.
...
완료 후 에이전트는 생성된 파일 목록을 오케스트레이터에 보고한다.

### Agent 3 — AI 동작 지침 작성 `(model: haiku)`

에이전트 정의: `{팀_위치}/agents/ai-behavior.agent.md`
출력: `{CWD}/instruction/AI_BEHAVIOR.md`
나머지 프롬프트는 Agent 1과 동일 패턴.

### Agent 4 — 커밋 컨벤션 작성 `(model: haiku)`

에이전트 정의: `{팀_위치}/agents/commit-convention.agent.md`
...
Agent 2가 보고한 `LANGUAGE_GUIDELINES_*.md` 파일 목록을 확인하여 composer 프롬프트에 포함한다.
...
```
{팀_위치}/agents/claude-md-composer.agent.md를 읽고 그 지침에 따라 작업한다.
