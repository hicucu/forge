...
워크플로우가 만들어낸 결과(push·PR·comment·tag·dispatch)가 다시 어떤 워크플로우의 트리거가 되면 순환이 발생한다. 핵심 판단 한 줄: **"이 잡이 생산하는 이벤트가 어떤 워크플로우의 트리거 목록에 들어가고, 그 이벤트가 GitHub 기본 보호를 우회하는가?"**
...
`GITHUB_TOKEN`으로 만든 push·PR·comment·tag 이벤트는 **새 워크플로우 실행을 트리거하지 않는다** (GitHub 기본 무한루프 방지 장치). 따라서 루프는 **이 보호가 깨지는 경로**에서만 발생한다:
...
4. 아래 차단 가드 중 최소 하나를 삽입해 순환 경로를 끊는다.
...
3. **토큰 확인** — 각 생산 행위의 토큰을 본다. `secrets.GITHUB_TOKEN`인가, `secrets.PAT_*`·`secrets.*_TOKEN`·deploy key·App 토큰인가? `actions/checkout`의 `token:`·`persist-credentials`도 확인.
4. **순환 매칭** — 생산 이벤트(2)가 트리거 목록(1)에 들고 보호 우회 조건(위 표)에 해당하면 **루프 후보**로 표시. 자기참조와 A→B→A 상호참조 모두 검사.
5. **가드 존재 확인** — 후보마다 아래 가드가 경로를 끊는지 본다.
...
- **토큰 출처를 안 봄** — `checkout`의 `token:`과 push 액션의 `github_token:`/`token:`을 모두 확인해야 PAT 사용을 잡는다.
