...
> **경로 규칙**: 모든 산출물은 `_workspaces/{workspaceName}/` 하위에 저장한다.
...
**부분 재실행**: "이슈만 수정해줘"
→ Phase 0에서 `_workspaces/review-report.md` 감지 → Phase 5만 실행
...
7. **workspace 명명 정책**: feature-pipeline은 `_workspaces/{workspaceName}/` (기능명 기반), `/code-review` 단독 커맨드는 `_workspaces/review-{branch-slug}/` (브랜치명 기반)을 사용한다. 두 경로는 접두사로 구분되므로 충돌 없음.
