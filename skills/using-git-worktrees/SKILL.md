...
**시작 시 공지:** "using-git-worktrees 스킬을 사용하여 격리된 작업 공간을 설정합니다."
...
**Submodule guard:** `GIT_DIR != GIT_COMMON`은 git 서브모듈 내부에서도 true. "이미 worktree에 있다"고 결론 내리기 전에 서브모듈 내부가 아닌지 확인:
...
> "격리된 worktree를 설정할까요? 현재 브랜치를 변경으로부터 보호합니다."
...
3. **기존 전역 디렉토리 확인:**
   ...
   ls -d ~/.config/forge/worktrees/$project 2>/dev/null
   ```
...
전역 디렉토리(`~/.config/forge/worktrees/`)는 확인 불필요.
...
# 전역: path="~/.config/forge/worktrees/$project/$BRANCH_NAME"
