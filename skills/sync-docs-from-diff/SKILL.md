...
2. 산출물은 `<CWD>/_workspaces/` 하위에 단일 디렉토리로 모이며, 후속 호출 시 재사용/누적된다.
...
**선행 조건**: `_workspaces/03_apply_log.md`가 존재해야 한다. 해당 파일이 없으면(사용자가 모든 패치를 거부해 적용 내역이 0건인 경우 포함) 검증 에이전트를 호출하지 않고 "적용된 변경 없음, 검증 생략" 보고 후 Phase 5로 진행한다.
