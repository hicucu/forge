...
4개 전문 reviewer를 병렬로 실행하고 결과를 통합하여 `review-report.md`를 작성한다.
Critical 이슈가 있으면 issue-fixer-agent를 파일별로 호출한다.
...
변경이 없으면 "리뷰할 변경사항이 없습니다" 반환 후 종료.
...
```
Agent(subagent_type="review-aggregator-agent"):
...
- 파일별로 issue-fixer-agent 호출:

```
# 각 이슈 파일에 대해 순차 실행
Agent(subagent_type="issue-fixer-agent"):
  target-file: {파일 경로}
  issues: {해당 파일의 이슈 발췌}
...
- 코드 파일 직접 수정 (issue-fixer-agent 위임)
