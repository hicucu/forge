...
"SessionStart": [
  {
    "hooks": [
      {
        "type": "command",
        "command": "\"C:\\Users\\USERNAME\\.claude\\plugins\\cache\\hicucu\\forge\\VERSION\\hooks\\run-hook.cmd\" session-start",
        "timeout": 10000
      }
    ]
  }
...
> `USERNAME`은 Windows 사용자명, `VERSION`은 설치된 버전(예: `1.0.0`)으로 변경합니다.
...
│   ├── run-hook.cmd       # Windows/Unix 크로스플랫폼 래퍼
